const findIndex = (items, beginWord, endWord) => {
  ...
  const beginIndex = items.findIndex(
    (item) => item.LineContent.toLowerCase() .includes(lowercasedBeginWord)
  );

  // Check if endWord exists in the array and find its index
  const endIndex = endWord
    ? items.findIndex(
        (item) => item.LineContent.toLowerCase().includes(lowercasedEndWord)
      )
    : -1;
